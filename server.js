...
mongoose
  .connect(
    "mongodb+srv://rabih:123@cluster0.jjhrj.mongodb.net/bookland?retryWrites=true&w=majority"
  )
...
app.get("/books/search/author/:id", function (req, res) {
  ...
  console.log(author);
  Books.find({ author: { $regex: author } })
    .then((result) => res.send(result))
    .catch((err) => console.log(err));
});
...
app.get("/books/search/any/:id", function (req, res) {
  ...
  Books.find({
    $or: [{ title: { $regex: search } }, { author: { $regex: search } }],
  })
    .then((result) => res.send(result))
    .catch((err) => console.log(err));
});
